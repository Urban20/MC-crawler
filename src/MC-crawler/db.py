...
cursor = conec.cursor()
cursor.execute('CREATE TABLE IF NOT EXISTS servers(ip PRIMARY KEY,pais TEXT,version TEXT)')
...
def buscar_version(version : str):   
    ...
    try:
        cursor.execute(f'SELECT ip FROM servers WHERE version LIKE ?',(f'%{version}',))
...
def buscar_pais(pais : str):
    ...
    try:
        cursor.execute(f'SELECT ip FROM servers WHERE pais = ?',(pais,))
